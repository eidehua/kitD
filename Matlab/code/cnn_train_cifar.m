function [net, info] = cnn_train_kitD(net, imdb, getBatch, varargin)
...
for epoch=1:opts.numEpochs
  ...
  filename = 'net-train';
  modelFigPath = fullfile(opts.expDir, char(strcat(filename,'.pdf')));
%   for m = 1:100
%     if exist(strcat([opts.expDir '/' filename],'.pdf'), 'file') ~= 2 
%       modelFigPath = fullfile(opts.expDir, char(strcat(filename,'.pdf')));
%       break
%     else
%       filename = char(strcat(filename,num2str(m)));
%     end
%   end
  if opts.continue
